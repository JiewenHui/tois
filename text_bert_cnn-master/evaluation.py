...
def mrr_metric(group):
	...
	rr=candidates[candidates["label_ids"]==1].index.min()+1
	if rr!=rr:
		return 0
	return 1.0/rr

def mrr_metric_10(group):
	group = sklearn.utils.shuffle(group,random_state =132)
	candidates=group.sort_values(by='score',ascending=False).reset_index()[:10]
	rr=candidates[candidates["label_ids"]==1].index.min()+1
	if rr!=rr:
		return 0
	return 1.0/rr
...
def map_metric(group):
	...
	ap=0
	candidates=group.sort_values(by='score',ascending=False).reset_index()
	correct_candidates=candidates[candidates["label_ids"]==1]
...
def evaluation_plus(modelfile, groundtruth=qa_path):
	answers=pd.read_csv(groundtruth,header=None,sep="\t",names=["question","answer","label_ids"],quoting =3)
...
def eval(predicted,groundtruth=qa_path, file_label_ids=False):
	if  'Windows' in platform.system() and file_label_ids ==False:
...
def evaluationBypandas_AP(df,predicted):
	df["score"]=predicted
	AP= df.groupby("question").apply(map_metric)
	return AP

def evaluationBypandas_MAP(df,predicted):
	df["score"]=predicted
	map= df.groupby("question").apply(map_metric).mean()
	return map

def evaluationBypandas_MRR10(df,predicted):
	df["score"]=predicted
	mrr= df.groupby("question").apply(mrr_metric_10).mean()
	return mrr
...
def precision_per(group):
	group = sklearn.utils.shuffle(group,random_state =132)
	candidates=group.sort_values(by='score',ascending=False).reset_index()
	rr=candidates[candidates["label_ids"]==1].index.min()
	if rr==0:
		return 1
	return 0
...
if __name__ =="__main__":
	...
	train=pd.read_csv(train_file,header=None,sep="\t",names=["question","answer","label_ids"],quoting =3)
