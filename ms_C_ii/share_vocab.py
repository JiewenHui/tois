...
class QA_quantum(object):
    ...
    def create_placeholder(self):
        ...
        self.input_y = tf.placeholder(tf.float32, [self.batch_size,self.label], name = "input_y")
    ...
    def add_embeddings(self):
        ...
        with tf.name_scope("embedding"):
            if self.is_Embedding_Needed:
                ...
                W_complex=tf.Variable(self.Position_Embedding(self.embedding_size),name = 'W',trainable = self.trainable)
            else:
                W = tf.Variable(tf.random_uniform([self.vocab_size, self.embedding_size], -1.0, 1.0),name="W",trainable = self.trainable)
                W_complex =tf.Variable(self.Position_Embedding(self.embedding_size),name = 'W',trainable = self.trainable)
            self.embedding_W = W
            W_complex = tf.tile(W_complex,[1,self.embedding_size])
            self.embedding_W_complex=W_complex
            self.overlap_W =  tf.get_variable('overlap_w', shape=[2, self.embedding_size],initializer = tf.random_normal_initializer())#tf.Variable(tf.random_uniform([3, self.extend_feature_dim], -1.0, 1.0),name="W",trainable = True)
            # self.position_W = tf.Variable(tf.random_uniform([31,self.embedding_size], 0,+2*math.pi),name = 'W',trainable = False)

        self.embedded_chars_q,self.embedding_chars_q_complex = self.concat_embedding(self.question,self.q_position)
        print(self.embedded_chars_q)
        self.embedded_chars_a,self.embedding_chars_a_complex= self.concat_embedding(self.answer,self.a_position)
    ...
    #         self.represent = self.narrow_convolutionandpool_real_imag(tf.expand_dims(self.M_qa_real,-1),tf.expand_dims(self.M_qa_imag,-1))
    ...
    #         W = tf.get_variable(
    #             "W_output",
    #             shape = [self.hidden_num, self.label],
    #             initializer = tf.contrib.layers.xavier_initializer(),
    #             regularizer=regularizer)
    #         b = tf.get_variable('b_output', shape=[self.label],initializer = tf.random_normal_initializer(),regularizer=regularizer)
    #         self.para.append(W)
    #         self.para.append(b)
    ...
    def concat_embedding(self,words_indice,position_indice):
        ...
        self.embedding_chars_q_phase=tf.nn.embedding_lookup(self.embedding_W_complex,words_indice)
