...
# from NNQLM_I import NNQLM_I as model
import random
import evaluation
import pickle
import config
...
def predict(sess, cnn, test, alphabet, batch_size, q_len, a_len):
    ...
    for data in batch_gen_with_single(test, alphabet, batch_size, q_len, a_len, overlap_dict=None,name='QA_quantum'):
        feed_dict = {
            cnn.question: data[0],
            cnn.answer: data[1],
        ...
        with sess.as_default(), open(precision, "w") as log:
            ...
            for i in range(FLAGS.num_epochs):
                ...
                datas = batch_gen_with_point_wise(train, alphabet, FLAGS.batch_size,
                                                  q_len=q_max_sent_length, a_len=a_max_sent_length,name='QA_quantum')
                # datas = batch_gen_with_point_MS(train, alphabet, FLAGS.batch_size,
           ...
           #     log.write('num_params = '+str(num_params)+ '\n')
                ...
                predicted_test = predict(
                    sess, cnn, dev, alphabet, FLAGS.batch_size, q_max_sent_length, a_max_sent_length)
