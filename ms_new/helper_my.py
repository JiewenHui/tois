...
def load_ms(dataset=dataset, filter=False):
    data_dir = "../new_data/" + dataset
    datas = []
    data = pd.read_csv(data_dir+'/train_.txt', header=None, sep="\t", names=[
                           "question", "answer", "flag"], quoting=3).fillna('N')
    datas.append(data)
    for data_name in [ "dev.txt",'testtt.txt']:
        print(data_name)
        data_file = os.path.join(data_dir, data_name)
        data = pd.read_csv(data_file, header=None, sep="\t", names=[
                           "question", "answer", "flag"], quoting=3).fillna('N')
...
def batch_gen_with_point_MS(df, alphabet, batch_size=10, overlap_dict=None, q_len=33, a_len=40,name='QA_quantum',now=0):
    # inputq inputa intput_y overlap
    pairs = []
    questions = []
    answers = []
    input_num = 8
    l,r = now,now+batch_size
    for i in range(l,r):
        row = df.iloc[i,:]
    ...
    for i in range(len(questions)):
        for j in range(len(questions)):
            if i ==j :
                ...
                label = [1,0]
            overlap=overlap_score(questions[i],answers[j])
            if overlap_dict == None:
                q_overlap, a_overlap = overlap_index(
                    questions[i],answers[j], q_len, a_len)
            else:
                q_overlap, a_overlap = overlap_dict[(
                    questions[i],answers[j])]
            pairs.append((questions[i], answers[j], label,q_position, a_position,overlap,q_overlap,a_overlap))
    yield [np.array([pair[i] for pair in pairs]) for i in range(input_num)]
