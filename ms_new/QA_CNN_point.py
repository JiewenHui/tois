...
class QA_quantum(object):
    ...
    def create_placeholder(self):
        self.question = tf.placeholder(tf.int32,[self.batch_size**2,self.max_input_left],name = 'input_question')
        self.answer = tf.placeholder(tf.int32,[self.batch_size**2,self.max_input_right],name = 'input_answer')
        self.input_y = tf.placeholder(tf.float32, [self.batch_size**2,2], name = "input_y")
        self.q_position = tf.placeholder(tf.int32,[self.batch_size**2,self.max_input_left],name = 'q_position')
        self.a_position = tf.placeholder(tf.int32,[self.batch_size**2,self.max_input_right],name = 'a_position')
        self.overlap = tf.placeholder(tf.float32,[self.batch_size**2,2],name = 'a_position')
...
#                 W_complex = tf.Variable(tf.random_uniform([self.vocab_size, 1], 0, 2*math.pi),name="W",trainable = self.trainable)
    ...
    def pooling_graph(self):
        with tf.name_scope('pooling'):
            ...
            self.represent_real_0 =tf.reshape(self.represent_real_0 ,[self.batch_size**2,-1])
            self.represent_real_1 =tf.reshape(self.represent_real_1 ,[self.batch_size**2,-1])
            self.represent_real_2 =tf.reshape(self.represent_real_2 ,[self.batch_size**2,-1])
            self.represent_real_3 =tf.reshape(self.represent_real_3 ,[self.batch_size**2,-1])
            # print(self.represent_real)
            ...
            self.represent_img_0 =tf.reshape(self.represent_img_0 ,[self.batch_size**2,-1])
            self.represent_img_1 =tf.reshape(self.represent_img_1 ,[self.batch_size**2,-1])
            self.represent_img_2 =tf.reshape(self.represent_img_2 ,[self.batch_size**2,-1])
            self.represent_img_3 =tf.reshape(self.represent_img_3 ,[self.batch_size**2,-1])
            w_0 = tf.Variable(tf.zeros(shape = [self.batch_size**2,tf.shape(self.represent_real_0)[-1]],name = 'W'))
            w_1 = tf.Variable(tf.zeros(shape = [self.batch_size**2,tf.shape(self.represent_real_1)[-1]],name = 'W'))
            w_2 = tf.Variable(tf.zeros(shape = [self.batch_size**2,tf.shape(self.represent_real_2)[-1]],name = 'W'))
            w_3 = tf.Variable(tf.zeros(shape = [self.batch_size**2,tf.shape(self.represent_real_3)[-1]],name = 'W'))
